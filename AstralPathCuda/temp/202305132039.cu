...
//现在开始全局内存书写
__global__ void test(int *h,int *b,int x) {
...
int main(int argc, char* argv[]) {
        ...
        int *d_x1;
        int *d_y1;
        cudaMallocHost((void **)&d_x1,M);
        cudaMemcpyAsync(d_x1,h_x,M,cudaMemcpyHostToDevice,stream_1);
        cudaMemcpyAsync(d_x1,h_x,M,cudaMemcpyHostToDevice,stream_2);
        cudaMallocHost((void **)&d_y1,M);
